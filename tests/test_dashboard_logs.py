...
def test_tail_filters_by_event_substring(log_file: Path) -> None:
    entries = [
        {"event": "HEALTH mode=live", "level": "info",
         "timestamp": _iso(2026, 4, 16, 10, 0)},
        {"event": "kalshi_ws_negative_qty ticker=X", "level": "warning",
         "timestamp": _iso(2026, 4, 16, 10, 1)},
        {"event": "HEALTH mode=live", "level": "info",
         "timestamp": _iso(2026, 4, 16, 10, 2)},
        {"event": "Placed", "level": "info",
         "timestamp": _iso(2026, 4, 16, 10, 3)},
    ]
    _write_lines(log_file, entries)
    client = TestClient(dashboard.app)
...
def test_tail_filters_by_level(log_file: Path) -> None:
    entries = [
        {"event": "info_event", "level": "info",
         "timestamp": _iso(2026, 4, 16, 10, 0)},
        {"event": "warn_event", "level": "warning",
         "timestamp": _iso(2026, 4, 16, 10, 1)},
        {"event": "err_event", "level": "error",
         "timestamp": _iso(2026, 4, 16, 10, 2)},
    ]
    _write_lines(log_file, entries)
    client = TestClient(dashboard.app)
...
def test_tail_skips_non_json_lines(log_file: Path) -> None:
    with log_file.open("w", encoding="utf-8") as f:
        f.write("not json\n")
        f.write(json.dumps({"event": "ok", "timestamp": _iso(2026, 4, 16, 10, 0)}) + "\n")
...
def test_stats_histogram_groups_by_event_prefix(log_file: Path) -> None:
    entries = [
        {"event": "kalshi_ws_negative_qty ticker=X", "level": "warning",
         "timestamp": _iso(2026, 4, 16, 10, 0)},
        {"event": "kalshi_ws_negative_qty ticker=Y", "level": "warning",
         "timestamp": _iso(2026, 4, 16, 10, 1)},
        {"event": "HEALTH mode=live", "level": "info",
         "timestamp": _iso(2026, 4, 16, 10, 2)},
        {"event": "Placed", "level": "info",
